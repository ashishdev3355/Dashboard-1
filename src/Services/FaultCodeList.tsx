...
const FaultCodeList = () => {
    ...
    const fetchFaultData = async (
        ...
        try {
            ...
            setFaultData([]);
            setTotal(0);
            setError("Failed to fetch fault codes. Please try again.");
        } finally {
            setLoading(false);
    ...
    const getSeverityBadge = (severity: SeverityLevel | number) => {
    const severityMap: Record<SeverityLevel, { label: string; color: string }> = {
    ...
    const config = severityMap[severity as SeverityLevel] || {
        label: "Unknown",
    ...
    const getDifficultyBadge = (difficulty: DifficultyLevel | number) => {
    const difficultyMap: Record<DifficultyLevel, { label: string; color: string }> = {
    ...
    const config =
        difficultyMap[difficulty as DifficultyLevel] ||
                ...
                {loading ? (
                    <div className="flex items-center justify-center py-20">
                        <div className="text-center">
                            ...
                            <p className="text-orange-500 text-xl">Loading fault codes...</p>
                                ...
                                <tbody>
                                    {faultData.map((item:any, idx) => (
                                        <tr key={item.id || idx} className="hover:bg-gray-50">
                                            ...
                                            <td className="border px-3 py-2">
                                                {item.company_id !== undefined ? item.company_id : "-"}
                                            </td>
                                            <td className="border px-3 py-2">
                    ...
                    <div className="text-center py-20">
                        <p className="text-gray-500 text-xl">No fault codes to display</p>
