...
import { Upload, FileSpreadsheet, CheckCircle, XCircle, AlertCircle, Database, Trash2 } from 'lucide-react';
...
interface DuplicateDetails {
    inFile: number;
    inDatabase: number;
    fileDuplicateDetails: string[]; // Changed to string[] as backend sends array of strings "group-command"
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-8">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-white">Live Data Commands Uploader</h1>
                            <p className="text-blue-100 mt-1">Import mechanic live data commands via Excel or CSV</p>
                        </div>
                    </div>
                </div>
                    ...
                    {!uploadResult && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                <h3 className="font-medium text-blue-900 mb-2">Expected File Headers:</h3>
                                <div className="flex flex-wrap gap-2 text-sm text-blue-800">
                                    <span className="bg-blue-100 px-2 py-1 rounded">model_group_id</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">command</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">description</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">short_name</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">decimals</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">min</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">max</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">units</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">type</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">formulaBased</span>
                                    <span className="bg-blue-100 px-2 py-1 rounded">referenceJSON</span>
                                </div>
                            </div>
                            ...
                            {(uploadResult.duplicates?.fileDuplicateDetails?.length > 0 || uploadResult.duplicates?.dbDuplicateCodes?.length > 0) && (
                                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                    ...
                                    {uploadResult.duplicates.fileDuplicateDetails?.length > 0 && (
                                        <div className="mb-4">
                                            <h5 className="font-medium text-yellow-800 mb-2">File Duplicates (Group-Command):</h5>
                                            <div className="text-sm text-yellow-700 space-y-1 max-h-40 overflow-y-auto">
                                                {uploadResult.duplicates.fileDuplicateDetails.map((dup, index) => (
                                                    <p key={index} className="font-mono">{typeof dup === 'string' ? dup : JSON.stringify(dup)}</p>
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {uploadResult.duplicates.dbDuplicateCodes?.length > 0 && (
                                        <div>
                                            ...
                                            <div className="text-sm text-yellow-700">
                                                <p className="mb-1">The following commands already exist in the database:</p>
