...
const FaultCodeSymptoms = () => {
    ...
    const fetchFaultData = async (
        ...
        try {
            ...
            if (result && Array.isArray(result.data)) {
                ...
                if (result.data.length === 0) {
                    setError(searchDtc || searchMake || searchGeneric !== "" 
                        ? "No clinical symptoms found for the specified criteria." 
            ...
            setFaultData([]);
            setTotal(0);
            setError("Failed to fetch symptom diagnostic stream. Please try again.");
        } finally {
            setLoading(false);
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-fadeIn pb-12 px-6">
            <PageHeader 
                title="Symptomatic Diagnostic Index" 
                subtitle="Visual and environmental indicators correlated with specific failure codes"
                icon={Stethoscope}
            />

            {/* Filter Section */}
            <Card title="Symptom Classification" icon={Filter}>
            ...
            <div className="space-y-6">
                {loading ? (
                    <div className="py-24 text-center">
                        ...
                        <p className="text-slate-400 font-bold italic tracking-widest animate-pulse">CORRELATING DATA STREAMS...</p>
                    </div>
                ) : faultData.length > 0 ? (
                    <Card title="Diagnostic Payload" icon={Activity} noPadding subtitle={`Known indicators: ${total}`}>
                                        ...
                                        <tr key={item.id || idx} className="hover:bg-primary-50/30 transition-colors group">
                                            ...
                                            <td className="px-4 py-4 max-w-lg">
                                                <div className="flex items-start gap-2">
                                                    ...
                                                    <p className="font-bold text-slate-700 leading-relaxed italic">{item.symptom || "-"}</p>
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 uppercase font-bold text-slate-400">
                                                {item.language || "EN"}
                                            </td>
                                            <td className="px-4 py-4 font-bold text-slate-600">
                                                {item.make || <Badge variant="secondary">Universal</Badge>}
                                            </td>
                    ...
                    <div className="py-24 text-center bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-400 font-bold italic">No symptomatic profiles found in current selection.</p>
                    </div>
                )}
